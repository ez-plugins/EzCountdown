...
public class EditStartMessageAction implements GuiAction {
    ...
    public EditStartMessageAction(CountdownManager manager, MessageManager messageManager, ChatInputListener chatInputListener, com.skyblockexp.ezcountdown.bootstrap.Registry registry) {
    ...
    public ActionResult handle(InventoryClickEvent event, Player player, String cdName, Optional<Countdown> countdownOpt) {
        ...
        chatInputListener.request(player, new StartMessageConsumer(player, cd, cdName, manager, messageManager, registry));
    ...
    private static final class StartMessageConsumer implements java.util.function.Consumer<String> {
        ...
        StartMessageConsumer(Player player, Countdown cd, String cdName, CountdownManager manager, MessageManager messageManager, com.skyblockexp.ezcountdown.bootstrap.Registry registry) {
            this.player = player;
            this.cd = cd;
            this.cdName = cdName;
            this.manager = manager;
        ...
        public void accept(String input) {
            ...
            com.skyblockexp.ezcountdown.api.model.Countdown newCd = new com.skyblockexp.ezcountdown.api.model.Countdown(cd.getName(), cd.getType(), cd.getDisplayTypes(), cd.getUpdateIntervalSeconds(), cd.getVisibilityPermission(), cd.getFormatMessage(), stripped, cd.getEndMessage(), cd.getEndCommands(), cd.getZoneId(), false, null, 0);
