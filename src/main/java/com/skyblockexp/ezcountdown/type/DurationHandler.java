...
public class DurationHandler implements CountdownTypeHandler {
    @Override
    public CountdownType getType() { return CountdownType.DURATION; }
    ...
    public void configureFromCreateArgs(Countdown countdown, String[] args, CountdownDefaults defaults) throws IllegalArgumentException {
        ...
        if (countdown.isRunning()) countdown.setTargetInstant(Instant.now().plusSeconds(secs));
    }
    ...
    public boolean tryApplyEditorInput(String input, Countdown countdown, Instant now) throws IllegalArgumentException {
        ...
        countdown.setDurationSeconds(secs);
        if (countdown.isRunning()) countdown.setTargetInstant(now.plusSeconds(secs));
        return true;
    }
}
